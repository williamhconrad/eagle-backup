...
def _molid_from_ligandname(name):
    """Recover the database id from a docking ligandname.

    Centroids/ZINC names look like ZINC000119504072_DE_174, where the id is
    the part before the first underscore. Enamine REAL names look like
    m_280710____43739026____31672232____31732540 and ARE the id already --
    splitting those on '_' yields just 'm' or 's', which matches nothing.
    """
    if name.startswith(("s_", "m_")):
        return name
...
def collect_info_block(infn, dbfn, index_column="zincid", fps_column="fp_hexstring"):
    ...
    if index_column in df.columns:
        ...
        df[index_column] = df['ligandname'].map(_molid_from_ligandname)
...
def collect_info_helper(inpdfn, dbfns, index_column="molecule_id", fps_column="fp_hexstring", mode="pbs"):
    ...
    if mode == "pbs":
        ...
        print(f"Using PBS clusters, {ncpus} cores:")
    ...
    if mode == "pbs":
        print("Number of PBS jobs:", len(joblist))
    ...
    if index_column in indf.columns:
        ...
        indf[index_column] = indf['ligandname'].map(_molid_from_ligandname)
...
def collect_info_fulldb(inpdfn, fps_path, index_column="zincid", mode="pbs"):
    indf = pd.read_feather(inpdfn)
    if index_column in indf.columns:
        pass
    elif index_column not in indf.columns and 'ligandname' in indf.columns:
...
def collect_info_realdb(inpdfn, fps_path, index_column="molecule_id", mode="pbs"):
    indf = pd.read_feather(inpdfn)
...
def collect_info_zinc22db(inpdfn, fps_path, index_column="molecule_id", mode="pbs"):
    indf = pd.read_feather(inpdfn)
    assert index_column in indf.columns or 'ligandname' in indf.columns, f"Cannot find column {index_column} nor ligandname in {inpdfn}"
...
def augment_results_fns_from_input(infns, mode="pbs", backup_path=None, overwrite=False):
    if len(infns) == 0:
        return
    if len(infns) == 1:
        infn = infns[0]
...
def augment_fn(infn, configfn, dbtype="cluster", mode="pbs", overwrite=False):
    ...
    if dbtype == "cluster":
        ...
        augment_results_fns_from_input(
            infns, mode="pbs", backup_path=None, overwrite=False)
        return
    else:
...
def augment_result_fns(i_iter, configfn, dbtype="cluster", mode="pbs", overwrite=False):
    config = load_configfn(configfn)
    resultspath = config['result_path']
...
def main():
    ...
    # dask never acquires workers on Crux (scheduler reports {}), so every
    # task returns None and pd.concat gets an empty list. Pool(20) works.
