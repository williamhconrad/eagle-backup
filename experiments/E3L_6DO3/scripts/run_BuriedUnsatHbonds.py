...
def run_top_pdbdir():
    ...
    prefix = f"substructure"
    outdir = os.path.join(config['project_path'], "top_pdbs_unsat", f"{prefix}_top{ntop}")
    params_path = os.path.join(config['project_tempdir'], 'top_params', f"{prefix}", f"params_top{ntop}")
    if not os.path.exists(params_path):
        raise Exception(f"{params_path} doesn't exist.")
    inpdbdir = os.path.join(config['project_path'], "top_pdbs", f"{prefix}", f"top{ntop}_pdbs")
    print(inpdbdir)
    ...
    for pdbfn in pdbfns:
        ...
        if not os.path.exists(paramsfn):
            ...
            continue
            #raise Exception(f"{paramsfn} not exist.")
        inarg = (zincid, pdbfn, paramsfn, outdir)
        #ret = BuriedUnsatHbonds_one_pdb(*inarg)
        inargs.append(inarg)
    ncpus = 16
...
def run_cluster_top():
    ...
    prefix = f"substructure"
    outdir = os.path.join(config['project_path'], "top_pdbs_unsat", prefix+"_cluster")

    params_path = os.path.join(config['project_tempdir'], 'top_params_cluster', f"{prefix}", f"params_top{ntop}")
    if not os.path.exists(params_path):
        raise Exception(f"{params_path} doesn't exist.")
 

    inpdbdir = os.path.join(config['project_path'], "top_cluster_pdbs", f"{prefix}", f"top{ntop}_pdbs_all_holo")
    print(inpdbdir)
    if not os.path.exists(inpdbdir):
        raise Exception(f"{inpdbdir} doesn't exist.")
    patt = os.path.join(inpdbdir, "*.pdb")
