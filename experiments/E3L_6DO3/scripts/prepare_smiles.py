...
def protonate_smiles_dir(smi_dir, outdir, mode='pbs'):
    ...
    if mode == 'pbs':
        cluster_obj = make_pbs_cluster(memory_per_worker="1GB",
                                       walltime="3:00:00",
                                       job_name="protsmi_worker")
        ncpus = max(1, min(100, int(len(smifns)//10)))
        adapt_cluster(cluster_obj, wait_count=400)
        client = Client(cluster_obj)
        print("Using PBS clusters, N_cpus", ncpus)
...
def prepare_smiles_real_db(i_iter, configfn, totalN, batchsize=2000, debug=False, mode='pbs'):
