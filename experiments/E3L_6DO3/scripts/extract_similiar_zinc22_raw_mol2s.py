...
def main():
    ...
    outpath = os.path.join(config["mol2_path"], f"substructure_mol2s", molid)
    logpath = "./logs"
    mode="pbs"
    
    dbfn_path = os.path.join(config["project_tempdir"], "substructure_mols", molid)
    patt = os.path.join(dbfn_path, "*.feather")
    dbfns = glob(patt)
    
    extract_mol2s_from_zinc22(dbfns, outpath, zinc22_mol2path, logpath, mode)
