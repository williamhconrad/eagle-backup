...
def save_top_N_params_from_df(df, config, ntop, outdir, overwrite=True):
    ...
    for i_iter in iter2ligandnames:
        ...
        outpath = os.path.join(outdir, f"params_top{ntop}", f"train{i_iter}")
...
def save_top_N_params(curr_iter, configfn, ntop, outdir, overwrite=True):
...
def run_save_top_N_params():
    ...
    config = load_configfn(configfn)
    ntop = 100000
    outdir = os.path.join(config['project_tempdir'], 'top_params', f'iter_{i_iter}')
    overwrite = True
    save_top_N_params(i_iter, configfn, ntop=ntop, outdir=outdir, overwrite=overwrite)
...
def run_save_top_cluster_params():
    ...
    ntop = 1000
    i_iter = 7
    outdir = os.path.join(config['project_tempdir'], 'top_params_cluster', f'iter_{i_iter}')
