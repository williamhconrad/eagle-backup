...
from .crux import make_pbs_cluster, adapt_cluster
import tarfile
...
def gen_params_from_folder(indir, outdir, mode='multiprocessing', 
    ...
    if mode == 'pbs':
        cluster = make_pbs_cluster(memory_per_worker="2GB",
                                   walltime="3:00:00",
                                   job_name="gen_params_worker",
                                   worker_extra_args=["--lifetime", "175m", "--lifetime-stagger", "4m"])

        n_workers = min( 300, int( len(mol2fns)*0.02 )+1 )
        if multimol2:
            n_workers = min( 300, int( len(mol2fns) ) )
        print(f"Using {n_workers} workers")
        adapt_cluster(cluster, wait_count=400)
